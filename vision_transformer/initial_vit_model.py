...
class VisionTransformer(nn.Module):
    ...
    def forward_features(self, x):
        ...
        x_total_pic=torch.empty(x.shape[0], 0, x.shape[2]).to(device)     #method initial
        ...
        for dimension in range(3):
            ...
            for slice in range(self.slices_num):
                ...
                #print(x_slice.shape)
                x_d_pic=torch.cat((x_d_pic,x_slice[:,1:1+self.patch_embed.num_patches,:]),dim=1)   #initial method
                '''if slice==0:
            ...
            if dimension==0:
                ...
                x_d=self.blocks_slice_attention_1(x_d)
            elif dimension==1:
                '''x_token_dimension=torch.empty(x.shape[0], 0, x.shape[2]).to(device)
                x_pic_dimension=torch.empty(x.shape[0], 0, x.shape[2]).to(device)
                for slice in range(self.slices_num):
        ...
        #x_total=torch.cat((x_total_token,x_total_pic),dim=1) #method_initial_2
        ...
        #can be improved
        #x_total=self.blocks_dimension_attention(x_total) method_initial_2
